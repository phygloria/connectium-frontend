...
import api from '@/services/api';  // api.js 파일을 가져옵니다.
import WeatherPopup from './WeatherPopup.vue';  // WeatherPopup 컴포넌트 가져오기
...
export default {
    ...
    setup() {
        ...
        const location = ref("중랑구");  // 사용자의 위치를 저장할 상태
        const error = ref(null);  // 에러 메시지를 저장할 상태
        const showPopup = ref(false);  // 팝업 표시 상태
        ...
        return { weatherData, location, error, showPopup };
    }
}
</script>
