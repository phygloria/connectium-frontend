              ...
              <div v-else class="list-container">
                <div class="list-column">
                  <div class="list-in-column">
                    <div class="list-card" v-for="content in paginatedCares" 
                    :key="content.id"
...
const paginatedCares = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage;
  const end = start + itemsPerPage;
  return cares.value.slice(start, end);
});

const totalPages = computed(() => Math.ceil(cares.value.length / itemsPerPage));
